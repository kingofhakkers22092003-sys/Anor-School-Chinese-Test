...
function speakChinese(button) { const phrase = button.dataset.text; if (!('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(phrase); utterance.lang = 'zh-CN'; utterance.rate = .77; window.speechSynthesis.speak(utterance); }
...
function makePdf(student) {
  ...
  results.forEach((result, index) => {
    ...
    stream.push(fill + ' rg', `57 ${y} 481 47 re f`, '0.77 0.58 0.22 rg', `57 ${y} 5 47 re f`,
      '0.13 0.22 0.29 rg', 'BT', '/F2 12 Tf', `75 ${y + 19} Td`, `(${pdfSafe(TEST_ORDER[index].label)}) Tj`, '/F1 9 Tf', '0 -13 Td', `(Chinese language skill ${index + 1}) Tj`, 'ET',
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('.play-audio').forEach(button => button.addEventListener('click', () => speakChinese(button)));
