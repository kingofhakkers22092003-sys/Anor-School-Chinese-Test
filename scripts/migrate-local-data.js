...
(async () => {
  const students = read('students.json', []); const questions = read('questions.json', {}); const settings = read('settings.json', {});
  for (const student of students) await db.saveStudent(student);
  for (const section of TEST_KEYS) for (const question of questions[section] || []) await db.addQuestion(question, section);
