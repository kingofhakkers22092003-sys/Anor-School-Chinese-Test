...
app.get('/api/students/:id', async (req, res) => {
  try {
    const student = await database.getStudent(req.params.id);
    if (!student) return res.status(404).json({ error: 'not-found' });
    maybeArchiveAttempt(student); await finalizeIfComplete(student); await database.saveStudent(student);
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    if (students.some(item => item.fullName.toLowerCase() === name && item.schoolClass === schoolClass)) return res.status(409).json({ error: 'duplicate' });
    const student = {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const student = (await database.getStudents()).find(item => item.fullName.toLowerCase() === fullName.trim().toLowerCase() && item.password === password);
...
app.patch('/api/questions/:section/:id', async (req, res) => {
  ...
  try {
    const bank = await database.getQuestionBank(TEST_KEYS);
    if (!bank[req.params.section].some(question => question.id === req.params.id)) return res.status(404).json({ error: 'not-found' });
    await database.updateQuestionGrade(req.params.id, normalizedGrade); res.json(await database.getQuestionBank(TEST_KEYS));
  } catch (error) { console.error(error.message); res.status(500).json({ error: 'server-error' }); }
});
...
app.get('/api/reading-passage', async (req, res) => {
  try { res.json((await getSettings()).readingPassage || { content: '', translation: '' }); }
  catch (error) { console.error(error.message); res.status(500).json({ error: 'server-error' }); }
});
...
app.post('/api/reading-passage', async (req, res) => {
  ...
  try { const settings = await getSettings(); settings.readingPassage = { content: content.trim(), translation: translation.trim() }; await database.saveSettings(settings); res.json(settings.readingPassage); }
  catch (error) { console.error(error.message); res.status(500).json({ error: 'server-error' }); }
});
